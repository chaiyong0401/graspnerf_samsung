...
def run(
    ...
    num_objects=5,
    # n=6,          ## graspnerf 
    n=1,            ## swindrnet, cleargrasp vgn 
    N=None,
    num_rounds=40,
    seed=1,
    sim_gui=False,
    rviz=False, ## rivz 창을 띄워 물체 파지하는거 확인할 수 있도록 함. 실제 이 코드에서는 쓰이지 않음. VGN 코드에서 사용
    round_idx=0,
    renderer_root_dir="",
    gpuid=None,
    args=None,
    ...
    output_modality_dict = {'RGB': 1,
                            'IR': 1,        ## swindr, cleargrasp
                            'NOCS': 0,
                            'Mask': 0,
                            'Normal': 1}    ## swindr, cleargrasp
    ...
    for n_round in range(round_idx, round_idx+1):
        ...
        while sim.num_objects > 0 and consecutive_failures < MAX_CONSECUTIVE_FAILURES:
            timings = {}
            # scan the scene
            # pc: high resolution tsdf(120)의 point cloud data 
            timings["integration"] = 0
            
            gt_tsdf, gt_pc, _ = sim.acquire_tsdf(n=n, N=N)     #################  simulation depth 만들기 
            ## gt_tsdf를 실제로 사용하지를 않음 
            if args.method == "graspnerf":
                grasps, scores, timings["planning"] = grasp_plan_fn(render_frame_list, round_idx, n_grasp, gt_tsdf) ## gt_tsdf가 실제로 쓰이지 않음 
            elif args.method == "swindrnet":
                state = State(gt_tsdf,gt_pc)
                grasps, scores, timings["planning"] = grasp_plan_fn(state)
            elif args.method == "cleargrasp":
                state = State(gt_tsdf,gt_pc)
                grasps, scores, timings["planning"] = grasp_plan_fn(state)
            else:
