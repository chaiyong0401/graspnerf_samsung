...
def blender_render(renderer, quaternion_list, translation_list, path_scene, render_frame_list, output_modality_dict, camera_focal, is_init=False):
    ...
    # baseline_file_path = os.path.join(path_scene, 'baseline_distance.txt')

    # # Write baseline distance to a file
    # with open(baseline_file_path, 'w') as file:
    #     file.write(f"Baseline Distance: {baseline_distance}\n")
    ...
    if output_modality_dict['IR'] or output_modality_dict['RGB']:
        ...
        for i in render_frame_list:  
            ...
            print("render/blender_render")
            print(f"baseline distance: {baseline_distance}")
            # render RGB image ## 실험 진행 중 사용된 내용
            ...
            # render IR image ## simulation depth를 만들기 위해 사용
    ...
    if output_modality_dict['Normal']:
        ...
        # # origianl material 저장
        # original_materials = {}
        # for obj in bpy.data.objects:
        #     if obj.type == 'MESH' and obj.name == 'tableplane':
        #         original_materials[obj.name] = obj.data.materials.copy()
        ...
        # renderer.src_energy_for_rgb_render = bpy.data.worlds["World"].node_tree.nodes["Background"].inputs[1].default_value
        # renderer.setCamera(quaternion_list[render_frame_list[-1]], translation_list[i], camera_fov, baseline_distance)
