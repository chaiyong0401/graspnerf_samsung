...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None, renderer_root_dir="", args=None):
        ...
        self.size = 6 * self.gripper.finger_depth       ## gripper.finger_depth = 0.05, size = 0.3
        # intrinsic = CameraIntrinsic(640, 480, 540.0, 540.0, 320.0, 240.0) # TODO: cfg   (width, height, fx, fy, cx, cy) focal length: 보통 이미지 센서에서 렌즈까지의 거리를 픽셀 단위로
        intrinsic = CameraIntrinsic(640, 480, 446.31, 446.31, 320.0, 240.0)
        self.camera = self.world.add_camera(intrinsic, 0.1, 2.0)
        self.rgb_datapath = "/home/dyros-recruit/GraspNeRF/src/nr/ckpt/image"
        logdir = 0
        self.output_root_path = os.path.join(args.log_root_dir, "rendered_results/" + str(logdir).split("/")[-1])   #./log/20240425-181946/rendered_results/0
        # self.output_root_path = os.path.join(args.log_root_dir, "rendered_results/" + str(logdir).split("/")[-1])
        

        ############ swindrnet 시행 ###########################################3
        python_path = '/usr/bin/python3'
        script_path = '/home/dyros-recruit/DREDS/SwinDRNet/inference.py'
        train_data_path = '/home/dyros-recruit/DREDS/SwinDRNet/data/DREDS/DREDS-CatKnown/train'
        val_data_path = '/home/dyros-recruit/DREDS/SwinDRNet/data/DREDS/DREDS-CatKnown/test_vgn'

        val_data_type = 'sim'  # 또는 'real'
        ...
        command = [
            ...
            '--rgb_depth_data_path', output_root_path
        ]
        self.command = command
        ######################################################33###################3

        # cleargrasp 시행 명령 ######################################################
        script_cleargrasp = '/home/dyros-recruit/cleargrasp/eval_depth_completion/eval_depth_completion.py'
        config_path = '/home/dyros-recruit/cleargrasp/eval_depth_completion/config/config.yaml'

        command_cleagrasp =[
            python_path,
            script_cleargrasp,
            '-c', config_path,
            '-rgbd', output_root_path
        ]
        self.command_cleagrasp = command_cleagrasp
        ########################################################
        self.args = args
    ...
    def acquire_tsdf(self, n, N=None):
        ...
        """
        if n ==1:           ## stereo matching을 이용한 simulation depth 생성 
            ### simulation depth from output_root_path
            output_root_path = self.output_root_path
            # print(self.method)
            # print(os.getcwd())
            # print(output_root_path)
            if self.method == "swindrnet":
                main_batch(output_root_path)        ## stereo matching을 통해서 손상된 simulation depth를 만드는 코드 
                print("simulation depth")
                command = self.command
                subprocess.run(command)
                # time.sleep(5)
                depth_img_path = "/home/dyros-recruit/GraspNeRF/src/nr/ckpt/restored_depth.exr"     ## 손상된 depth
                depth_img_restored =  cv2.imread(depth_img_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH) 
                # plt.imshow(depth_img_restored, cmap='gray')  # 회색조로 깊이 이미지 표시
                # plt.colorbar()  # 색상 막대 표시
                # plt.title('Depth Image_restored')
                # plt.show()
            else:
                main_batch(output_root_path)        ## stereo matching을 통해서 손상된 simulation depth를 만드는 코드 
                command_cleagrasp= self.command_cleagrasp       ## cleagrasp depth completion을 통해 depth 복원
                subprocess.run(command_cleagrasp)
                depth_img_path = "/home/dyros-recruit/GraspNeRF/src/nr/ckpt/restored_depth_cleargrasp.exr"     ## 손상된 depth
                depth_img_restored =  cv2.imread(depth_img_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH) 
                # plt.imshow(depth_img_restored, cmap='gray')  # 회색조로 깊이 이미지 표시
                # plt.colorbar()  # 색상 막대 표시
                # plt.title('Depth Image_restored_cleargrasp')
                # plt.show()

            theta_degrees =35         ## theta 조정
            theta = np.deg2rad(theta_degrees)
            # theta = np.pi / 6.0         ## original theta
        ...
        timing = 0.0
        rgb_datapath = self.rgb_datapath
        for idx, extrinsic in enumerate(extrinsics):
            depth_img = self.camera.render(extrinsic)[1]        # ground truth depth    n==1인 경우 22가 나온다. 
            # print(f"ground truth simulation depth size: {depth_img.shape}")

            # plt.imshow(depth_img, cmap='gray')  # 회색조로 깊이 이미지 표시
            # plt.colorbar()  # 색상 막대 표시
            # plt.title('Depth Image_blender_gt')
            # plt.show()
            ################################################################################
            rgb_img = self.camera.render(extrinsic)[0]

            rgb_filename = f"{rgb_datapath}_{idx}.png"
            depth_filename = f"{rgb_datapath}_{idx}.exr"
            save_rgb_image(rgb_img,rgb_filename)
            save_depth_image(depth_img,depth_filename)
            ######################################################################################
            
            if n ==1:
                tic = time.time()
                tsdf.integrate(depth_img_restored, self.camera.intrinsic, extrinsic)
                timing += time.time() - tic
                high_res_tsdf.integrate(depth_img_restored, self.camera.intrinsic, extrinsic)
            else:
                tic = time.time()
