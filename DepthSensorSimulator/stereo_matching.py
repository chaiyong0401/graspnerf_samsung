...
class DREDS(Dataset):
    def __init__(self, root, scale):
        ...
        for i in range(6):
            if not os.path.exists(root + "simDepthImage.exr"):
                # filename = f"000{4*i+2}.png"
                # depth_filename = f"000{4*i+2}_0.exr"  # 깊이 파일 이름
                filename = str(4*i+2).zfill(4) + ".png"
                depth_filename = str(4*i+2).zfill(4)+"_0.exr"
                self.ir_l_list.append(os.path.join(root,"ir_l",filename))
                self.ir_r_list.append(os.path.join(root,"ir_r",filename))
                self.depth_list.append(os.path.join(root,"depth",depth_filename))
                print(os.path.join(root,"depth",depth_filename))
    ...
    def __getitem__(self, index):
        ...
        # if index ==5:
        #     plt.imshow(depth_array, cmap='gray')  # 회색조로 깊이 이미지 표시
        #     plt.colorbar()  # 색상 막대 표시
        #     plt.title(f'before stereo mathcing Depth Image{4*index+2}')
        #     plt.show()
...
def main_batch(path):
    ...
    ###########################
    # scale = 0.5                             ## original
    # scale = 1                               ## real rgb와 맞춘 scale 0.455
    scale = 0.88                             # image scale with simulation pybullet 추가 조정이 필요할 수도
    ...
    # maxDisp = 10
    minDisp = 2                             # min disparity between left and right images
    f = torch.tensor(446.31)                # original focal length of the sensor
    # f = torch.tensor(540)                # focal length of the simulation sensor
    ...
    # for i, (imageLTensor, imageRTensor, ir_l_path, ir_r_path) in enumerate(DataLoader):
        ...
        for i in range(imageLTensor.shape[0]):
            ...
            save_dir = os.path.join(save_root,"stereo_depth")
            ...
            ###################### stereo mathcing 후 depth image #####################
            # if i ==5:
            #     plt.imshow(depthImageExr, cmap='gray')  # 회색조로 깊이 이미지 표시
            #     plt.colorbar()  # 색상 막대 표시
            #     plt.title(f'stereo_mathing_after transparent_Depth Image{4*i+2}')
            #     plt.show()
            ###########################################
